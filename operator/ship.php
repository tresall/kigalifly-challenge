    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <table id="table-prod" class="table table-hover table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                            $sel = "SELECT * FROM doctor, products, orders WHERE products.prod_id = orders.prod_id AND orders.doc_id = doctor.doc_id AND orders.received = 1 ORDER BY orders.order_date DESC";
                            $que = $c->query($sel);
                            if ($que->num_rows > 0) {
                                $no = 1;
